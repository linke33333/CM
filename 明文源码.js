...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, env) {
	if (sub) {
		...
		addresses = addresses.concat('172.67.173.4:443#CFnat');
		if (hostName.includes(".workers.dev")) {
			addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF节点'));
		} else {
			addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF节点'));
		}
	}
